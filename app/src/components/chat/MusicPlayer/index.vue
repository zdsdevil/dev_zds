...
export default {
  ...
  computed: {
    ...mapState({
      music_src: state => state.chat.music_src,
      music_start_time: state => state.chat.music_start_time,
      music_lrc: state => state.chat.music_lrc,
    })
  },
  methods: {
    ...
    updateTime(e) {
      ...
      const res = this.music_lrc.filter((t) => t.time < e.target.currentTime);
      let currentLrc = null;
