...
export default {
  ...
  methods: {
    nodeIsDialog(dom) {
      if(dom.classList.contains('ant-modal-root')) return true;
      if(dom.parentElement) return this.nodeIsDialog(dom.parentElement);
      return false;
    },
    bodyClose(e) {
      const isDialog = this.nodeIsDialog(e.target);
