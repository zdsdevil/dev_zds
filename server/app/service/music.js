// import { initMusicSheet, searchMusic } from '../utils/spider';
 ...
 */
    async initMusicList() {
        ...
        const musicCount = await this.ctx.model.Music.count();
        if (musicCount) {
            this.app.maxCount = musicCount;
            return console.log(`已经有${musicCount}首音乐了、无需初始化歌单了！`);
        }
        const musicList = await spider.initMusicSheet(params);
        await this.ctx.model.Music.insertMany(
            musicList,
    ...
    async search(params) {
        ...
        try {
            ...
            const res = await spider.searchMusic(url)
            if (res.code === 200) {
    ...
    async collectMusic(payload, params) {
        ...
        const music = { music_mid, music_pic120, music_artist, music_album, music_name, user_id }
    ...
    /* 获取热门歌曲 */
    async hot() {
        return await this.ctx.model.Music.find().lean();
    }
